...
from dataset.dataload_ocr import TextDataset, TextInstance
import json


class OCR_data(TextDataset):

    def __init__(self, root='/home/zhihui/Dataset/data_test', mode='label',is_training=True,transform=None):
    ...
    def parse_txt(mask_path):
        ...
        for i in range(len(info)):
            ...
            label = info[i]['label']
            # if label == "poi_val0" or label == "poi_val1":
            # if label == "poi_val0":
            #     val = info[i]['description']
            #     transcripts.append(val)
