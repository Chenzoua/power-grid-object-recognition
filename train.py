...
if torch.cuda.is_available():
    device_index = 0  # 修改为你希望使用的具体CUDA设备索引
...
def train(model, train_loader, criterion, scheduler, optimizer, epoch):
    ...
    for i, (img,pointer_mask, dail_mask, text_mask, train_mask, transcripts, bboxs, mapping) in enumerate(train_loader):
        ...
        with open("losses.txt", "a") as loss_file:
            loss_file.write(
                    'Epoch: {:d} Batch: {:d} Loss: {:.4f} pointer_loss: {:.4f} dail_loss: {:.4f} text_loss: {:.4f} rec_loss: {:.4f}\n'
                .format(epoch, i, loss.item(), loss_pointer.item(), loss_dail.item(), loss_text.item(),
                        loss_rec.item()))
        loss_file.close()
...
def main():
    ...
    train_loader = data.DataLoader(trainset, batch_size=cfg.batch_size,
                                    shuffle=True, num_workers=cfg.num_workers, pin_memory=True,collate_fn=collate_fn,generator=torch.Generator(device = 'cuda'))
