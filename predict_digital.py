...
for index in image_list:
    ...
    cv2.waitKey(0)
    image, image_info, digital_list, meter_list= det.detect(images, index)
    if len(digital_list) == 0:
        print("no detected number")
        continue
    else:
        for i in digital_list:
            im, image_info, number_list = det_nuber.detect(i, i)
            # cv2.imshow("index", i)
            # cv2.waitKey(0)
            # cv2.imwrite("pre_exp/number.jpg", i)
            for nub in number_list:
                ocr_res = ocr.ocr(nub, cls=True, det=False)
                for line in ocr_res:
                    print(line)
                ...
                cv2.waitKey(0)
                # cv2.imwrite("pre_exp/result_img.jpg", result_img)
