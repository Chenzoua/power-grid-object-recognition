...
class MeterReader(object):
    ...
    def __call__(self, image,point_mask,dail_mask,word_mask,number,number2,std_point):

        img_result = image.copy()
        value=self.find_lines(img_result,point_mask,dail_mask,number,number2,std_point)
        print("value",value)
    ...
    def find_lines(self,ori_img,pointer_mask,dail_mask,number,number2,std_point):
        ...
        # calculate value
        try:
            if number!=None and number[0]!="" and number2!=None and number2[0]!="":
                # print('number:',number,number2)
                two_value = float(number)-float(number2)
                # print(number[0])
                # print("towval",two_value)
            else:
                return None
        except ValueError:
            return None
        ...
        if std_ang * now_ang !=0:
            ori_value = float(number2)
            # print("ori_val",ori_value)
            value_sca = (two_value / std_ang)
            value= ori_value + (value_sca*now_ang)
        else:
            return None
