...
def compute_mask_loss(mask_batch, student_feature, teacher_feature, imitation_loss_weight):
    ...
    bs_m, na, height, width = mask_batch.shape
    bs_f = student_feature.size(0)
    # print('bs_f',bs_f)
    if bs_f == bs_m:
        bs = bs_m
        mask_batch_no = mask_batch.unsqueeze(4).expand(bs, na, height, width, student_feature.shape[-1])
    else:
        bs = bs_f
        mask_temp = mask_batch.unsqueeze(4)
        mask_temp = mask_temp.unsqueeze(3, -1, -1, -1, -1)
        mask_batch_no = mask_temp.expand(bs, na, height, width, student_feature.shape[-1])

    norms = mask_batch_no.sum() * 2

    # print('T', teacher_feature.shape)
    # print('S', student_feature.shape)
    # print('mask_', mask_batch_no.shape)
...
#     #
#     # mask_batch_no = mask_batch_no.expand(-1, 3, -1, -1, -1)
#     #
#     print('T', teacher_feature.shape)
#     print('S', student_feature.shape)
#     print('mask_', mask_batch_no.shape)
