...
def collate_fn(batch):
    img,  pointer_mask, dail_mask, text_mask, train_mask,boxes0,boxes1,transcripts = zip(*batch)
    bs = len(img)
    images = []
    pointer_maps = []
    dail_maps = []
    ...
    if len(transcripts) > 1 and len(boxes0) > 0:
        for index, gt in enumerate(zip(transcripts[0], boxes0)):
            for t, b in zip(gt[0], gt[1]):
                mapping.append(index)
                texts.append(t)
                bboxs.append(b)
    # 在访问元组之前添加长度检查
    if len(transcripts) > 1 and len(boxes1) > 0:
        for index, gt in enumerate(zip(transcripts[1], boxes1)):
            for t, b in zip(gt[0], gt[1]):
                mapping.append(index)
                texts.append(t)
                bboxs.append(b)
    # 处理数据
        ...
        print("Error: Invalid tuple lengths for transcripts or boxes1.")
