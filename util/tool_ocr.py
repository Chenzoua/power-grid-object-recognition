...
def collate_fn(batch):
    img,  pointer_mask, dail_mask, text_mask, train_mask,boxes1,transcripts = zip(*batch)
    bs = len(img)
    images = []
    pointer_maps = []
    dail_maps = []
    ...
    texts = []
    bboxs = []
    # for index, gt in enumerate(zip(transcripts[1], boxes0)):
    #     for t, b in zip(gt[0], gt[1]):
    #         mapping.append(index)
    #         texts.append(t)
    #         bboxs.append(b)
    for index, gt in enumerate(zip(transcripts[0], boxes1)):
        for t, b in zip(gt[0], gt[1]):
            mapping.append(index)
            texts.append(t)
            bboxs.append(b)
