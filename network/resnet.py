...
class ResNet(nn.Module):
    def __init__(self, name="resnet50", pretrain=True):
        ...
        if pretrain:
            ...
            # state_dict = torch.hub.load_state_dict_from_url(model_urls[name], model_dir="./cache")
            # base_net.load_state_dict(state_dict)
            # base_net.load_state_dict(model_zoo.load_url(model_urls["resnet50"], model_dir="./cache"))
            base_net.load_state_dict(torch.load('Model_all/cache/resnet50-19c8e357.pth', map_location=torch.device('cuda')))

            # base_net.load_state_dict(torch.load('cache/resnet50-19c8e357.pth', map_location='cuda:0'))
        # print(base_net)
